...
bool LlamaChat::complete(const std::string& prompt, const std::string& schema,
    ...
    //
    // 2026-09-12 加的。原来调用方写死一个数（8192，后来 12288），而这个
    // 数和 schema 要的输出量是两条各自在变的线：一章从"一堆段落"改成
    // "几场戏"之后，每场八个必填字段加几十段正文，段数下限又提过一次，
    // 输出量一路涨。两次撞上上限的表现**都不是"写短了"而是"解析失败"**
    // ——输出停在半截 JSON 上，报"找不到合法 JSON"，那一章落成 0 字，
    // 看报错根本想不到是这儿。第一次靠 8192→12288 糊过去了，第二次又撞上。
    //
    // 与其再拍一个魔数，不如让它跟着上下文走：提示词多长是已知的，
    // 剩下的全给输出。留 64 个 token 的余量——模板尾巴和收尾那几个 token。
    //
    // 算出来还是不够写的话，别让它带着一个负数往下走——采样那个循环
    // 会一轮都不转，回一个空串，而空串在上层看着像"模型什么都没说"。
    // 掉进下面那条"提示词太长"，那句话人能读懂。
