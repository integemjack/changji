...
std::string missing_for(Capability c, const NodeFacts& f) {
    switch (c) {
        case Capability::Llm:
            ...
            // ⚠️ **本机写不了文，这一版没有那条路了。** 进程内那条大模型
            // 2026-09-14 删了：写文一律走外部服务。
            //
            // 这儿原来还接着判"编没编 llama.cpp"和 `[models].llm` 那个权重
            // 文件，两条在这一版上都不作数了：llama.cpp 编进来只说明配音那
            // 半能跑，而那个权重文件已经没有任何东西会去读。照旧判的话，一
            // 台只装了配音模型的机器会在那张表上亮一格绿的"能写文"，而它一
            // 个字都写不出来——**这种算多了不会在任何地方报错**，人照着那
            // 张表去配，只会一直查不出来为什么写不动。
            return "写文要指到远端服务（[llm].backend = remote 加地址）"
                   "——进程内那条已经删了";
