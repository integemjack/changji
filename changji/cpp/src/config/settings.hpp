...
struct LLMConfig {
    ...
    /// 流水线最费的就是文字：一部剧几十万 token。手上已经有订阅的人，走
    /// 命令行等于这部分不再额外花钱。
    ///
    ...
    /// 一集十七镜、每镜十几个字段的结构化 JSON，模型到 300 秒还在写。
    ///
...
/// 一集默认多长（秒）。
///
/// 60 秒是这个项目一直在用的那个数（用户 2026-09-16 给 hulian-test 填的
/// 就是它），也是短剧一集的常见长度。**它只决定装配时怎么切**，不影响
/// 一章写多长——那由这一章的内容定。
inline constexpr double kDefaultEpisodeS = 60.0;
...
struct AssemblyConfig {
    ...
    /// **每集多长（秒）。填了就是「章模式」，0 = 老的一集一章。**
    ///
    /// 一集多长（秒）。一章按它自己的内容写完、拍完，最后按这个数切成几集，
    /// 能切出几集是这一章内容的结果。
    ///
    /// 用户 2026-09-16 的判词：剧本应该从一章里直接拿，写多长由这一章的
    /// 内容定；分镜出片之后再按固定的每集时长切成几集，能切出几集是内容
    /// 的结果。原来是反过来的——剧本提示词里写着「这一集总时长约 N 秒」，
    /// 不够模型就凑、超了就压，分镜那一步还会再按目标时长挤一次。
    ///
    /// **2026-09-16 起这不再是个开关，只是「一集多长」。** 用户当天定的：
    /// 只留章模式，集模式删掉。原来 0 表示"走老的一集一章"，于是同一步
    /// 养着两套提示词和两条代码路径，内容大半重合、各写各的会飘。
    ///
    /// 老配置里显式写着 0 的，读进来时抬到默认值（见 settings.cpp 里
    /// normalize_episode_s）——那些项目原来就没在按它切集，抬上去只是让
    /// 装配那一步有个数可用，不会改变已经出好的片子。
    double episode_s = kDefaultEpisodeS;
    /// 中文字幕单行上限，全角字符数
