...
struct Shot {
    ...
    /// 需要和前后镜保持一致的细节。
    ///
    /// 提示词第 16 条在教它写（prompts.toml：「记录需要与前后镜保持一致的
    /// 细节，比如道具在哪只手」），schema 里有、`kLlmShotFields` 里有、
    /// `validate()` 查它不超 200 字，存进 shots.json，`/api/shots` 发出去，
    /// 单集页的镜头抽屉里摆成「接戏」那一行（2026-09-15 补上的——在那之前
