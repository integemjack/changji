...
7. 每一章的 hook 是**这一章结束时悬着的那件事**：悬念、反转，或者一个
   明确的情绪落点。后面每一集的结尾都要落在这种位置上。
8. hook_after 填**那个钩子前面最后一句的原文，照抄十到二十个字**。
   程序要靠它在正文里定位这一刀切在哪。抄错了或者自己编，就只能退回
   切在章尾。
9. chapter_id 照抄下面给的，不要改、不要漏、不要自己编号。
10. 节选里带「……」的地方是中间省略掉的正文，不要把省略号当成情节。
