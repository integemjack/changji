...
// story.json，再写成剧本、排成分镜、配成音、渲成片——**一整条
// 流水线为一段复读机跑了一个多小时**。
//
...
/// 守卫拿来比对「这两句是不是同一句」的那个形态：去掉首尾空白和包裹的引号。
///
/// **导出它是因为摘除那一头必须用同一个。** 2026-09-16 撞到过：复读守卫
/// 按这个形态数，而 chapter_write 的摘除用自己那个 `bare()`（连句内的逗号
/// 句号一起剥掉）。同一句「他微笑，嘴角上扬。」——守卫量到 9 个字、够着
/// 8 的下限，数到三次判废；摘除量到 7 个字、够不着下限，一次都不摘。
/// 于是那一句**按构造就是救不回来的**：守卫必判、摘除永远不碰，
/// 十分钟写出来的一章直接作废，重试只是再掷一次骰子。
///
/// 两边共用这一个，摘完之后守卫数到的每一句都只剩一次，必过。
std::string repeat_key(const std::string& s);
