...
/// 一场戏的末尾值多少。**比有说法的钩子还高一档。**
///
/// 一场戏是一个完整的戏剧单元（谁想干什么、谁拦着、局面变成什么），
/// 它的末尾天然就是一集该收的地方——2026-09-12 起 AI 写正文就是一场一场
/// 写的，一章分几场基本等于分几集。切在场中间的话，一集会停在一件事的
/// 半当中，下一集开头接的是半场戏。
///
/// 让到半个容量：一集长出或短掉一半还能接受，把一场戏劈两半不能接受。
inline constexpr double kSceneSlack = 0.5;
...
///    优先级从高到低是**场的末尾 > 有说法的钩子 > 无名的段落边界**，
///    各自有各自的让路余地（kSceneSlack / kNamedHookSlack）。一集最好停在
///    一场戏演完的地方，其次停在一个读懂剧情标出来的悬念上，最后才是
///    一个只保证"不在半句话中间"的段落边界。
