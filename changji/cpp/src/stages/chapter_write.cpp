...
ordered chapter_schema(int target_scenes, int paras_per_scene) {
    ...
    const ordered schema = [&] {
        ...
        scene_props["last_line"] = {
            {"type", "string"},
            {"description",
             "这一场的最后一句：上面那个 turn 发生的**那一刻**。三种形状挑一种，一章里几场不要都用同一种——①一个动作停在半当中（手伸到一半、门推开一条缝）；②一句说出口的话被打断或者没人接；③一样东西刚被看见（一行字、一张脸、一个数字）。这一刻要是有人开口，就把那句话**原样写出来、带引号**，不要写成「谁说完了什么」这种转述，读者要听见的是那句话本身。写完它这一场就结束了：不要回头总结，也不要解释这一句是什么意思，悬念靠留白不靠解释"},
            {"minLength", 12},
            {"maxLength", 120}};
