...
// ⚠️⚠️ **本地这条路上，下面这些 description 模型一个字都看不到。**
//
// 2026-09-12 晚查实的。local_client.cpp 把提示词和 schema 分两路传：
// 提示词原样喂给模型，schema 只走 `json_schema_to_grammar()` 变成 GBNF。
// **而 GBNF 里只有结构**——键名、顺序、minItems/maxItems/minLength/
// maxLength。description 在转换时就被丢掉了。
//
// 实测核过：把整份提示词落盘，3037 个字里搜不到 "description"、
// "minLength"、"这一场的正文"、"至少三成" 里的任何一个。
//
// 这件事的后果比它看起来大：
// · 描述里那些写得很用心的手艺话（"至少三成的段落要有人说话"、
//   "turn 要拍得出来"、"长短要错落"）**从来没有生效过**；
// · 之前好几轮把效果归因给"schema 描述"的结论，机制上不成立，
//   那些数八成是单跑方差；
// · 所以真正管得住模型的只有两样：**提示词里的话**，和**语法里的形状**。
//   想让它照做一件事，要么写进提示词，要么变成 min/max。
//
// 描述先留着不删：外部大模型那条路（client_http）是把 schema 整个塞进
// 请求的，那边 description 是有用的。改描述之前先想清楚你在改哪条路。
ordered chapter_schema(int target_scenes, int paras_per_scene) {
    ...
    //     而 minItems 是 GBNF 硬约束——语法里没有"结束数组"这个选项，
    ...
    const ordered schema = [&] {
        ...
        scene_props[kChapterBodyField] = {
            ...
            // **它在明令禁止短对白**。「"别碰。"」只有 5 个字，语法不让它
            // 收引号，模型只能挂一个动作或者一句体感上去才凑得到 20。
            //
        ...
        //
        // 下限 12 → 4（2026-09-12 晚，理由同 paragraphs）：一集的收口
