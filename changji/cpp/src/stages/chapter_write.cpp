...
ordered chapter_schema(int target_scenes, int paras_per_scene) {
    ...
    const ordered schema = [&] {
        ...
        scene_props[kChapterBodyField] = {
            ...
            //
            // **每段最长 150 字。2026-09-12 从 300 降下来的。**
            //
            // 300 是照着"真实网文最长的段也就三百来字"定的——那是**最长**
            // 的那一段，不是常态。而 maxLength 是这里唯一能管住段长的东西：
            // 实跑量出来段长中位 99 字，而量过的真实网文是 33、传统小说是
            // 60，我们自己算段数用的 kCharsPerParagraph 是 45。三倍的差。
            //
            // 描述里早就写着"一段推进一两秒钟的事：一个动作、一句话、
            // 一次看见……长短要错落"，模型不听——这条在这个项目里验证过
            // 很多遍：**管得住的从来不是措辞，是它没得选。**
            //
            // 长段还是凑字数最好藏的地方：放松 minItems 之后模型立刻改成
            // "少写几段、每段写长"，段长中位从 70 涨到 99。
            //
            // 代价是真实网文里偶尔那个三百字的长段写不出来了。先按中位数
            // 对齐——那是差得最远的一项。
