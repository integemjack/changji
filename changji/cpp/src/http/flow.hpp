...
//     assets / episode 四条，2026-09-11 合并时把它并进了「这一集」）。谈不上
...
//     比不说更糟。
//
// 「这一集」这一格的判据取的是**装配出片子**（见 flow.cpp 里那段），发不发
// 是可选的收尾动作，算进来只会让这一格永远不打勾。
...
/// 因为那两页也合成一页了（前端今天是「这一集」下面的 `EpShots`）。
///
