...
int main(int argc, char** argv) {
    ...
    // **跳过也算没过。**
    //
    // 原来这里写的是"跳过的不算失败：Python 没起、项目路径没给，
    // 都是环境问题不是契约问题"。那句话本身没错，但结论错了：
    // 环境问题确实不是契约问题，可它意味着**这一条根本没验**，
    // 而"没验"不该退 0。
    //
    // 实测过最坏的样子：什么后端都没起的时候，它报
    // 「一致 0，不同 0，跳过 9」然后退 0。三个数字里两个是 0，
    // 退出码也是 0，任何自动化看了都会当成过了。
    //
    // 这套工具自己的原则是"跳过等于放弃检查"（见方案第六节，
    // 那 4 条跳过是专门去挣掉的）。既然如此，退出码就该跟着这条原则。
    if (tally.skipped > 0) {
        std::cout << "\n有 " << tally.skipped << " 条没验成，算没过。\n"
                     "**「跳过」不是「通过」**——它只说明这一条没跑起来，"
                     "不说明两边一致。\n"
                     "常见原因：后端没起来、项目路径给错了、语料里有没实现的"
                     "前置步骤。\n";
        return 1;
    }
    return 0;
