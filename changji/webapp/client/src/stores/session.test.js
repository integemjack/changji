 ...
 * 「现在在做哪部剧的哪一集」这一份。
 *
 * 为什么单给它写用例：整个界面的每一页都从这儿取项目路径和集号，而
 * `refresh()` 是全应用被叫得最勤的一个函数（换项目、换集、每个带
 * `refresh: true` 的动作、还有好几处手动调）。它里面那道**过期闸**一旦漏
 * 掉，后果不是画错一个数——上一部剧的 flow 会连着 `selectEpisode` 一起落
 * 到新这一部头上，还写进 localStorage，于是新这一部的每一页都拿着一个不
 * 属于它的集号去问引擎。这种错在界面上长得像"引擎抽风"，很难联想到这里。
 *
...
describe('session', () => {
  ...
  it('回来晚了的那趟不许写：上一部剧的 flow 不能盖到这一部上', async () => {
    const s = useSession()
    let landA
    ...
    landA()
    await slow
    // 不判过期的话，这三条会全变成 A 的——集号还会被写进 localStorage，
  ...
  it('过期那趟的报错也不算数：不能把新这一部清掉', async () => {
    ...
    await slow
    expect(s.project.title).toBe('B')
    expect(dispatched.some((e) => e.detail?.includes('上一部剧没了'))).toBe(false)
  })
})
