 ...
 * 时候想起来要对照另一部电影，这一来一回就把当前这一页的状态丢了。
 * 常驻之后：**点一下就换，人还停在原来那一页**。
 *
 * 每条只放两样：片名，和到哪一步了。路径、画风、镜头数都不在这儿——
 * 那些是「这一部电影的详情」，归项目页；这条栏只回答「切到哪一部」。
 *
 * 2026-09-14 重排，按同一条规矩（每次打开都要回答的问题才留在页面上）：
 *
 *   · **按阶段排，同阶段按最近动过。** 原来只按 mtime 倒序，而 mtime
 *     界面上一个字都不显示——顺序有依据但看不见，而且建一个测试壳就把它
 *     顶到正在做的那几部前面（实见：6 条里 4 条是 refresh-test / glm-demo /
 *     321 / smoke-tmp 这种残留）。按阶段排之后顺序有了看得见的理由：
 *     第二行那句话就是排序键，空壳自动沉底。
 *   · **每条能改名、能删。** 原来删一个空壳要先点中它（当前项目和章号
 *     一起被换掉）→ 走到项目页 → 删 → 打一遍名字 → session.clear() 之后
 *     一个项目都没选 → 再点回自己那部片、再挑回那一章。四个空壳四轮。
 ...
 * 而引擎那头可能正在给这部电影写分镜。实测：假引擎报着「正在给 ep02 出分镜」，
 ...
 * 收没收起，**故事页和别处各记各的**。
 *
 * 故事页默认收起：那一页是个编辑器，而这条栏列着另外几部电影——你正在写
 ...
 * （出了 3/12 章 = 25），拿它排的话正在发片的会沉到「还没分镜」（45）
...
/**
 * 跑完一章、写完一批之后，卡上那句「到哪一步了」要跟上。
 *
 * **盯的是"跑完"这个下降沿，不是 projectPath。** 原来那条 watch 盯的是
 * 换项目，而注释写着「建了、删了、跑完一章之后列表要跟上」——跑一章根本
 ...
 * 「1/2 章已出片」原样挂着。上面那段话说的正是这个症状，它只是没等到人。
 *
 ...
 * 它们：写一章正文、落成章节、提人物、删章都是同步 POST，一个都不占槽。
 ...
 * 当前这部电影的「走到哪一步」有没有变。
 *
 * **上面那两条都盖不住留在同一部电影里干活的情况。** 下降沿那条只认 Run /
 * Write 两个作业槽；换项目那条要真的换走一次。而改动进度的路大多是同步
 * POST、一个槽都不占——落成章节、提人物、写一章、删章、存分镜。人就待在
 * 这一部电影上点完，卡片上那句话原样挂着。
 *
 * 实测（2026-09-15）：点「落成章节」，引擎 `POST /api/story/episodes` 回
 * 200、9 条都建好了，而右边卡片仍写着「正文 4/8 章，还没落成章节」，那颗
 ...
 * 改名输入框的键都会冒泡上来：
 *   · 在改名框里打空格 → preventDefault 把字符吞掉（打不出带空格的片名），
 ...
 * 没聚焦过就永远不会 blur，那一行会一直挂着编辑框而不是片名，收起再展开
...
async function remove(p) {
  ...
  removing.value = ''
  confirmName.value = ''
  // 删的是当前这一部才清上下文——删别的项目时人还在自己那部电影里干活。
