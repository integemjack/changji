...
/* **这条读数是顶栏里唯一可以整块不要的东西，所以它先走。**
   它 `flex: none` 又 `nowrap`，一让不让地占着 409px；顶栏里被挤的是
   `.nav`（`min-width: 0` + 横滚，缩起来没有下限）——而导航是一直要用的，
   读数只是看一眼。原来这条写的是 1000px，**取早了**：在「这一集」那一页
   顶栏还多一个集号选择器，实测全摆下要 1152px（brand 30 + 项目名 102 +
   导航 235 + 集号 111 + 读数 409 + 引擎灯 101 + 两颗图标 68 + 八道
   间隙 64 + 左右内边距 32），于是 1000–1152 这一段里读数在、导航被裁，
   正在看的那一格「这一集」尾巴被切掉。项目名越长要得越多（`.proj`
   上限 11rem），所以往上留一点余量取 1200。 */
