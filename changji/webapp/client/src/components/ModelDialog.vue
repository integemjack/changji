...
const scopeHint = computed(() => {
  ...
  return (
    '挑哪一档是这部电影的设置，跟着项目目录走，派到别的机器上也照这份来。' +
...
const scopeLabel = computed(() =>
  session.projectPath ? '这一档记在这部电影里' : '还没选项目 · 这次只写全局',
)
 ...
 *
 * 镜头抽屉、角色抽屉、场景抽屉、以及「这部电影」那个窗早就有这道拦截，
 * 措辞照它们（「…有改动还没保存，关掉就没了」）。
 */
function tryClose() {
  ...
  <div v-if="open" class="mask" @click.self="tryClose">
    <section v-if="g" ref="panel" class="dlg" tabindex="-1">
      ...
      <div class="dlg__meta">
        ...
        <!-- **挑哪一档 = 这部电影的；文件在哪儿 = 这台机器的。** 两件事在同
             一个窗口里发生，不说一声的话没人分得出来。摆成一行两端对齐，
             左边这台机器、右边这部电影——并排才读得出是一对，原来一个在右上角
             一个在下一行，看着像两件不相干的事。 -->
        <div class="meta">
