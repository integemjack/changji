          ...
          <template v-else>
            <label class="field">
              ...
              <!-- ⚠️ **别再写「它会出现在项目库里」。** 项目库那条栏只列
                   ...
                   这句话原来写的是「接进来只对这一次有效」，那**说重了**：
                   路径是记在 localStorage 里的（session.selectProject），
                   刷新、关标签页再打开都还在。真正会丢的只有"切到别的项目
                   之后"。说成"只对这一次"会让人以为刷新就没了，于是白白
                   再打一遍那条长路径。 -->
              <span class="field__hint">
                得是一个已经建好的项目目录（里面有 project.json）。
                <strong v-if="!inWorkspace">它不在项目库目录下：接进来之后它就是当前项目
                （刷新也还在，路径记在这台机器上），但切走就找不回来了——栏上不会有
                它那一条，也没有「最近打开」，要回来得再打一遍这条路径。</strong>
              </span>
            </label>
          </template>
        </div>
