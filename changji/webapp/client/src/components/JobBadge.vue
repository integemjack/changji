 ...
 * 界面就会悄悄把排队的全算成在跑的。
 *
 * ⚠️ 长跑任务（出片、写全片）不在此列：一种只有一个槽，同种再点当场被拒
...
onMounted(() => {
  ...
  // 渲染——那时候这块就只能显示路径尾巴了，而"哪部电影在跑"正是它要回答的。
...
const KIND = {
  ...
  // 它）：展开剩下 N 章、写全片、以及**批量补分镜**（/api/plan/all）。写死
  // 「写正文」的话，补分镜跑着的时候顶栏说的是「写正文」，
  // 而它右边那句引擎现说的消息写着「正在给 ep02 出分镜」
  // ——同一行里两种说法。而按下补分镜那一头只弹一句「正在给 … 补分镜」，
  // 进度就得来顶栏这一格看。
  //
  // 三条路的 message 都自带主语（「正在写 ch03（第三章）」「正在写第 3 章」
  // 「正在给 ep02 出分镜」），所以这一格退回**类别**就够，具体干什么由它说。
  // 引擎那边今天分不出来：三条 start 传的 episode_id 都是空串，stage 也只有
  // 出片那一族在报——真要分，得先给这个槽加一个能认的字段。
  //
  // 页面仍然指 /story：三条里两条（展开正文、写全片）在那儿，补分镜那条在
  // 故事页的一键链里也按得到（镜头屏上还有一颗）。分不出来的时候指中多数
  // 那一个。
  ...
  // 梗概在故事页那个框里想（`/story` 的「想几个给我挑」），不在这一章里。
  // 原来指着 /episode 是照着"剧本大纲"那一页写的，那一页 2026-09-11 删了。
  //
  // ⚠️ **预告片这一条今天指的地方是空的。** 它原来落在设定页「分集」那一格
  // 的折叠区（剪的按钮、时长下拉、剪完那份只活在内存里的草稿），而那一格
  // 2026-09-18 整个下了——`/api/script/trailer` 引擎还在，界面上已经没有入口
  // 按得起来它。指 /assets 是"离它最近的那一页"，不是"它在那儿"；真要接
  // 回来的话，入口和这一行一起定。
 ...
 * 点一行就过去。
 *
 * **先切项目再跳页**：任务可能是另一部电影的，不切的话跳过去看到的是当前
 * 这部电影的同名页面——比不跳更误导。
 */
