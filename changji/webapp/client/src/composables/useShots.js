 ...
 * 于是同一镜在两页上说法不一样。
 *
 * 2026-09-10 两页合成一页（今天是「这一章」下面的 `EpShots`）之后，
 * 这里就是唯一的那份。
 ...
 * 回来接着排，这是很正常的用法。
 *
 * 换项目或换章时清空（见下面那个 watch）——那时候这些 shot_id 已经
 * 不属于当前这一章了，留着只会在别的章上点亮几个不相干的格子。
 */
...
export function useShots() {
  ...
  // 不订它的话，那一轮给这一章补完分镜，墙上还是空的——人要切一次 tab
  ...
  async function load() {
    ...
    try {
      ...
      // `没有章节 <id>`，也就是这个章号根本不存在。
      //
      // 于是吞掉 404 等于：章号对不上（localStorage 里记着一个已经被删的
      // 章、或者别的标签页刚删掉它）时，界面和"这一章还没分镜"长得一模一样，
      // 一声不吭，底下还摆着「AI 出分镜」——点下去是给一个不存在的章排
  ...
  watch(
    ...
    () => {
      // 换项目或换章：队列里那些 shot_id 不属于这一章了，留着只会在
      // 别的章上点亮几个不相干的格子。见 sent / waiting 的注释。
   ...
   * 由界面决定画走马灯还是画具体进度。
   *
   * 别用 inflight 里的 step/total：那是整章的位置（第 21 镜 / 共 22 镜），
   ...
   * 开跑。
   *
   * `ids` 为空是整章全流程。非空是只跑那几镜；再给 `stages` 就只跑那几段
  ...
  async function startInner(ids, stages, force, allEpisodes) {
    ...
    //
    // 整章出片时点亮的是**引擎接下来会挨个跑的那些**——也就是还没到终态
    ...
    try {
      res = await api.run({
        ...
        // 整章那条默认不 force（接着没跑完的往下跑）；「全部重出」
        // 会显式传真，否则那个按钮点了什么都不会发生。
        force: force === null ? one : force,
        // 整个项目一起跑。引擎见到它就**忽略 episode_id**，把每一个有分镜
        // 的章排进队列（run.cpp：「做的就是量产，一章一章手点没有意义」）。
        // 这一章已经出完、别的章还差着的时候，主按钮走的就是这条。
  ...
  // 引擎重启之后自己回来：这一页停在「读不到这一章的分镜」上时，
  // 那份表一回来就重读一趟。见 useRetryWhenBack。
  useRetryWhenBack(() => loadError.value, load)

  // 批量那条跑完也重拉一次。只订下降沿：跑的过程中它一章一章地写，
  // 而这一页要的是"这一章的分镜出来了没有"。
  //
  // `session.refresh()` 也要跟着叫，理由同下面出片那条：顶栏那个章号下拉
  // 上写着每一章多少镜、侧边那几个对勾也按分镜算，而批量补分镜正好把这两
  // 样都改了。不叫的话它们停在开跑之前，直到下一次换章或者干完点别的。
  // ⚠️ **这儿原来盯的是 `writeStore.running`，而这一页没有人驱动它。**
  // `useWriter` 的轮询只有故事页和设定页会开；批量补分镜起在"写"那个槽上，
  // 而这一页上那个旗子从头到尾是假的，下降沿一次都不来。上面那段话说的正是
  // 这件事。
