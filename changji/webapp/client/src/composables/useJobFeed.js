 ...
 */
export async function openJobFeed(streamId, consume, onLost) {
  let sock = null
  let opened = false
  const take = (msg) => {
  ...
  await new Promise((resolve) => {
    sock = openJobSocket(
      ...
      () => {
        ...
        if (opened) onLost?.('和引擎的连接断了，这件事干没干完不好说')
        resolve()
      },
      () => {
        opened = true
  ...
  const loop = async () => {
    ...
    while (!closed) {
      ...
      try {
        ...
        if (!closed) onLost?.(e?.message || '取不到进度了，这件事干没干完不好说')
        return
      }
      if (closed) return
      // 信箱没了：没开过，或者太久没人来取被扫掉了。**别傻等。**
      if (!r || r.exists === false) {
        onLost?.('和引擎的连接断了，这件事干没干完不好说')
        return
      }
      since = r.next ?? since
      let over = false
