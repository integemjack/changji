<script setup>
/**
 * 发布。「这一章」那一页的第四格——引擎带着投递那一层时才出现（见
...
// 单条还是一批。整部跑完是八章八条片，一条一条填标题投出去，
// 投到第五条人就开始出错——投重、漏投、标题串章。
const mode = ref('one')
...
const warnings = computed(() => {
  ...
  // **只有挑的是这一章的成片时，这个数才说得上话。**
  //
  // `plannedDurationS` 是顶栏选中那一章的排布时长（/bff/flow 按
  // episode_id 算的），而片单是**全项目**的——挑了别章那一条还拿它比上限，
...
async function loadAll() {
  // 这一趟是给哪部电影/哪一章读的。换项目、换章都会叫它，两趟叠在一起时
...
/**
 * 换了一部电影：挑中的那几条片、勾上的那一批，都得松开。
 *
 * `rel` 是**项目内的相对路径**（`output/ep01.mp4` 这种），两部片里长得
 * 一模一样是常态。不松开的话：
 *
 *   · 批量那一批留着上一部勾的那几条，而下面按钮上写的还是「投递 3 条」
 *     ——真发出去的是新这一部里**同名**的那几条成片，人一条都没看过；
 *   · 单条那个 `rel` 同理，而且 loadAll 只在它为空时才挑默认那条，于是
 *     新这一部默认选中的不是这一章的成片。
 *
 * 投递是发到外面去的，投错了不像存盘那样还能改回来。换章不清：片单是
 * 全项目的，人可能正挑着别的章那一条。
 */
...
/**
 * 标题和简介没填过就拿这一章的兜底。**人改过的不覆盖。**
 *
 * ⚠️ 判据不能只是"空不空"：那样从 ep01 换到 ep02，框里留着的是 ep01 的
 * 标题（它不空），于是 ep02 会带着上一章的名字投出去——而投递是发到外面
 * 去的，错了不像存盘那样还能改回来。
 *
 * 记一下上次自动填的是什么：框里还是那一份就说明人没动过，换章时换成新
 * 这一章的；人改过了（和记下的不一样）就一个字不动。
 */
        ...
        <section class="sec stack">
          ...
          <label v-else class="field">
            ...
            <input
              ...
              class="input"
              placeholder="留空用章名；可用 {title} 章名 {episode} 章号 {index} 序号"
            />
          </label>
