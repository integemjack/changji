 ...
 *   · **这一章有镜头拿不到参考图**。出图模型是图像编辑模型时，没有编辑源
 *     它会退化成文生图、出来是彩色噪点，而闸门那条「不是空图」拦不住
 *     （方差比真图还大）。一镜两分钟，跑完再说就是一章雪花加一个钟头。
 ...
 * 主按钮写着「这一章出完了」，而这一章**一个视频都没有**——按钮还顺手把
 * 它跳过去，人永远等不到它被重跑。
 *
 ...
 * **全项目**还没出片的镜头数（含这一章）。
 *
 * 不用再发一趟请求：`/api/project` 每一章本来就带 `shots` 和 `status`
 ...
 * 全项目还有几章没分镜。
 *
 * 一章有没有分镜看 `shots` 是不是 0——`/api/project` 每一章本来就带这个数，
...
/**
 * 整部电影还有事要做吗：有章没分镜，或者有镜头没出片。
 *
 * **主按钮靠它变成「跑完整部电影」**（见 runWholeShow）。2026-09-17 之前
 ...
 *
 * 2026-09-17 在只能写文的 Mac 上撞见：这一章那一页三颗按钮全灰，悬停写着
 * 「还不能开工：进程内配音 · 缺模型 [models].tts」——**缺一个配音模型，
 * 把拆分镜也一起拦了**。而「跑完整部电影」前两步（没剧本的改编、没分镜的
 * 补上）跟配音、跟出图都没有关系：同一时刻我从接口起的那批分镜正跑得
 * 好好的，体检自己也写着「能产什么：写文、装配」。
 *
 * 后果是纯文字那台机器上，整部电影的分镜只能一章一章点「AI 重出分镜」——
...
/**
 * 工具行左边那行读数。**只剩这一章多长**：镜数在 tab 上，画幅在项目页
 * 「这部电影」那一行，三处说同一件事的时候留最不重复的那一份。
 *
 ...
 *
 * 槽里两种东西，对不上的代价不一样：出好的镜头是 `object-fit: contain`
 * 的播放器，比例对不上是**留黑边**（写死 9:16 的话，横屏项目的每张牌都是
 * 上下两条黑、中间一小条画面）；还没出片、只有首帧的那几格是
 * `object-fit: cover` 的图，对不上是**裁**——16:9 的首帧塞进 9:16 的槽，
 * 左右各切掉三分之一还多。
 *
 * **读不到画幅那一支按 16:9，不按 9:16。** 2026-09-18 新建项目默认改成
 * landscape 之后，`/bff/project/video` 读砸一次就够了——换项目那一下的
 * 竞态、老引擎没这条接口、网络抖一下，`video` 都会是 null。回落写竖屏，
 * 等于让现在**最常见**的那种项目一读失败就撞进上面那个故障。两个值都是
 * 猜，挑这个是因为它至少是引擎发给新项目的那一个，不是这一页自己编的。
 *
 * **回落只有这一份。** CSS 那边 `aspect-ratio` 直接读 `--cell-ratio`、不
 * 再自带默认值：同一个默认值搁两处，改一处就漏一处，而 `.wall` 上这个属
 * 性总有值——它就是这个 computed，每一支都返回一串比例。
 */
...
/**
 * 跑完整部电影：没剧本的先改编、没分镜的补上、再把还差的镜头出完。
 *
...
async function generate() {
  ...
  // 换一章的话，发出去的是**上一章的剧本配这一章的章号**，整张分镜表就是
  // 照着别人的剧本拆的，而且一个字都不会报错：长度、台词、场景看着都对，
  // 只是不是这一章的故事。设定页那条「一键出图」早就这么钉了，理由一样。
  const project = session.projectPath
  const episodeId = session.episodeId
  ...
  if (!scriptData?.script?.trim()) {
    ...
    // 不上、这一章不在了）一个字都没有。run() 每次进来先把 error 清空，
    // 所以这会儿它装的就是这一趟的。
    ui.warn(
      error.value
  ...
  if (result) {
    ...
    // 出短了要说出来。60 秒的章出过两镜六秒——提示词里"合计 16 个镜头"
 ...
 *   · **换了章**：shot_id 是 `ep01_s03_sh007` 这种、带着章号前缀（见引擎
...
watch(
  ...
  () => {
    ...
    // **抽屉也要跟着关。**
    //
    // 换章之后 `openShot` 找不到那个 shot_id 了（id 带章号前缀），模板上
    // 那道 `v-if="draft && openShot"` 会把抽屉藏起来——看着像关了。可
    // `draft` 和 `openId` 还在：`draftDirty` 照样算成"有改动"，于是在**新
    // 这一章**点第一个镜头，弹出来的是「这一镜有改动还没保存，切走就没
    // 了」——说的是一个已经看不见、还属于上一章的镜头。
    openId.value = ''
    draft.value = null
  },
)
 ...
 * 两分钟；构图不对的话视频跑得再好也是白跑。一章二十几镜先把首帧铺开，
 * 扫一眼哪几镜不对、改完提示词再出片，比整章跑完再返工省得多。
 *
...
async function loadDoctor() {
  ...
  // 的说明。上一部电影的结论落到这一部头上，表现是这一部明明能跑却被拦住
  // （或者反过来），而理由写的是另一台机器/另一部电影的事。
...
async function loadVideo() {
  ...
  // 换片时慢的那趟后落地，画幅就是上一部的——而这一页每个格子的长宽比
  // 就是按它排的（cellRatio）
  const want = session.projectPath
  try {
...
async function loadPreview() {
  ...
  try {
    ...
    // bareShots），也就是"这一章能不能开工"的一半判据：读不到的时候
...
watch(
  ...
  () => {
    loadVideo()
    // 体检里有一项跟着这部电影的画幅走，换片要重查。这一格被 KeepAlive
 ...
 * **切到别的格子就把键盘让出去。**
 *
 * 这一页被「这一章」的 `<KeepAlive>` 冻着——切到剧本 / 成片，组件是
...
<template>
  <div class="stack stack--lg">
        ...
        <button
          ...
          :title="
            (textOnlyTodo
              ...
              : blockedWhy) ||
            (showTodo
              ? `一路跑完整部电影：没剧本的先改编、没分镜的补上、缺的参考图画好，还差的 ${todoLabel} 一次跑完——中途不用回来点；跑起来之后这颗按钮自己变成「停下」`
              : '每一镜都有片了；点了会全部重出')
          "
    ...
    <section v-if="(blocked || previewError) && shots.length" class="sec">
      ...
      <div class="stack stack--sm">
        ...
        <!-- **拿不到参考图的那几镜。** 引擎算的（stages::shots_without_refs），
             按下去 post_run 也会 400——这儿先说，省得人等一个钟头拿到一章
    ...
    <template v-else>
      <!-- 镜号带：一格一镜，颜色是状态，点了跳到那一镜。
           **原来是按时长铺的时间轴**（长镜头占的格子宽），151 镜的时候
           每格只剩 2~5px，"哪几镜特别长"这个信号本来就没了，而镜号叠成
           一片墨迹。用户 2026-09-17：「用小方块，支持换行」——等大的方块
           换行排，每个镜号都读得出来，时长交给指上去那句提示。 -->
      <div class="timeline">
...
.timeline__bars {
  ...
  min-width: 0;
  display: flex;
  /* **换行。** 一章一百多镜，不换行只有两条路：挤成认不出的墨迹，
     或者横着滚——前者是原来那样，后者就没法一眼看完整章了。 */
  flex-wrap: wrap;
  gap: 3px;
  /* **封个顶，超了在自己框里滚。** 宽屏上 151 镜正好五行，碰不到这个顶；
     而 375px 上一行只放得下六格，151 镜就是 26 行七百多像素——整屏被它
     吃掉，真正要看的镜头格全被推到屏幕外面去了。 */
  max-height: calc(24px * 5 + 3px * 4);
  overflow-y: auto;
...
/* 等大的小方块。**容得下三位数**：151 镜那章最后一格写的是「151」，
   10px 字号下大约 16px 宽，给到 24 见方还剩点余量。 */
.tl {
  border: none;
  border-radius: 3px;
  padding: 0;
...
/* 方块等大之后镜号一律显示——它本来就是这一格的身份，藏起来就只能靠
   指上去问了。 */
...
.cell__frame {
  ...
  display: block;
  width: 100%;
  /* 比例由 .wall 上的 --cell-ratio 定，读不到画幅走哪一支见 cellRatio
