 ...
 *     场景   哪儿    什么空间、什么光、什么色
 *
 * 原来还有第三格「分集」（怎么切、每集多长、每一集停在哪），2026-09-18
 * 连同 AssetEpisodes.vue 一起下了：一章就是一部分、整章都在，没有"切"这件事。
 *
 * **这一页的活只有一件：看图对不对。** 2026-09-14 按项目页那条规矩重排：
 * 每次打开都要回答的问题才留在页面上。落下来是三层——
 *
 *   · **tab 就是状态行**：「角色 2 · 缺 6」「场景 25 · 缺 25」。站在角色
 *     格也知道场景格一张都没画。
 *   · **右上一排是流水线**：理解故事 → 出图。两颗都是
 *     页级动作，原来「照故事定妆」在角色格和场景格各有一份、按的是同一个
 *     接口。「覆盖已有」一个勾管两个按钮——对理解和出图它是同一个意思：
 *     已有的也顶掉。勾上，理解变「重新理解」、出图变「全部重画」。
 *   · **格子里只剩图和一个搜索框。** 刷新（页面订着 refs 频道，画完自动
 *     刷）、只看缺图（一键出图本来就把缺的全画了，没人需要手动找）、卡片上
 *     和图说同一件事的 pill（0/3、有空景图）——都删了。
 *
 * **分成 tab 不是分成两页。** 两格共享同一个项目、同一份故事，切 tab 不该
 * 换地址、也不该重新拉一遍——它们是同一件事的两个面。tab 记在
 * `?tab=` 上，刷新和分享链接才停在原地。
 *
 ...
 * 就照故事出，没有就退回"第一章有内容的剧本"，两样都没有才 400
 * 「还没有剧本」。而新项目缺的是**故事**——那句话
 * 把人支去一个更靠后的步骤（剧本是理解故事写出来的），而这一页自己的空状态
 ...
 *
 * 侧栏那个对勾（`done.assets`）的判据是：有角色、场景都登记了、**而且这一章
...
/**
 * 理解故事。**一颗按钮，一件活**：提结构 → 定长相 → 落成章节 → 逐章写剧本，
  ...
  if (
    ...
    !confirm(
      `会把 ${count} 张参考图全部重画，手传上去的也会被顶掉。` +
        ...
        // 会把**全项目**已经渲染好的镜头退回待跑。一部已经出过片的电影，
        // 这句话的分量比"大约 8 分钟"重得多。
        `已经渲染好的镜头也会退回重跑。` +
...
watch(
  ...
  (now, before) => {
    ...
    if (left && q.by_hand) {
      ...
      else ui.info(`那一部电影画好了 ${made} 张${tail}`)
    }
  },
)
...
async function loadStory() {
  ...
  // 哪一部电影的那一趟。在项目库里连着点两部，慢的那趟后落地，「理解故事」
  // 那颗按不按得动就是照上一部算的（canUnderstand 数的是有没有章有正文），
  // 而两个格子自己读的是新的——同一屏上两套数。
  ...
  try {
    ...
    // 没有故事的老项目走到这儿是正常的：没正文时「理解故事」那颗自己会说
    story.value = null
  }
}
 ...
 * 还是上一部的角色和场景。于是从 A 点到 B 的那一两秒里，B 的标签上明晃
 * 晃写着「角色 2 · 缺 6」「场景 25 · 缺 25」——而 B 可能一个角色都
 * 没有。两个格子自己读的是新的，同一屏上两套数。
 *
...
function loadAll() {
  ...
  if (want !== shownFor) {
    ...
    // 「这两秒里在项目库点了另一部电影，`overwrite` 那一下就带着"会冲掉"的
 ...
 * 长活跑完，这一行标签也要跟上。
 *
 * 两个 tab 上那几个数（「角色 2 · 缺 6」「场景 25 · 缺 25」）正是理解故事
 * 和一键出图动出来的。不订的话，跑了一个多小时回到这一页，标签还是开跑
 * 之前的数，而且不会自己变。
 *
 * 只订下降沿：跑的过程中它一个个写，这一行是给"到哪一步了"看的，不是
 * 进度条。
 */
/**
 * ⚠️ **这儿原来盯的是 `writer.running`，而这一页不保证有人在驱动它。**
 * `useWriter` 的轮询只有故事页会开（这一页一个驱动它的地方都没有——原来
 * 那个「分集」格 2026-09-18 下了）。一上来落在「角色」格、或者刷新
    ...
    <template v-else>
      <nav class="tabs">
        ...
        <!-- 流水线：理解故事 → 出图。用户 2026-09-17 定的：设定页只有这两颗，
             「出图」要等理解完才出现。
             （顶栏「这一章」原来也要等图画齐，2026-09-18 改成"理解完就有"——
             剧本是理解的产物，等图画齐等于让人看不到刚写好的剧本就先去画一
             小时的图。判据见 App.vue 的 visibleSteps。）
      ...
      <!-- KeepAlive：切回来时还停在原来展开的那个角色上。两格各自都有
