...
onBeforeRouteLeave(() => {
  ...
  if (!n) return true
  return confirm(`有 ${n} 个角色改了还没保存，走了就没了。确定？`)
})
          ...
          <div class="chr__cols">
            ...
            <div class="stack">
              ...
              <!-- **折起来。** 这是排障用的——出来的图不对时，翻开看一眼
                   ...
                   三分之一的高度，而抽屉是用来改描述的。 -->
              <!-- ⚠️ **「每个镜头」有一个例外，而且正好是最会来这儿排障的
                   那一种。** 大特写（ECU）那一档，引擎**整个不拼角色层，也
                   不带参考图**（prompt_compose 里 `insert_shot` 那一段：实
                   跑出来"身份层在前→全身人像……整个去掉→前景手机、背景虚化
                   的人，唯一对得上分镜的那张"）。不说的话，一个大特写画错了
                   人，用户翻开这一折看到提示词是对的，就会去怀疑别处——而那
                   一镜根本没收到这串字。 -->
              <details
                class="fold"
                title="每个镜头拿到的都是这一串，逐字节相同。只有大特写（ECU）那一档例外：那一档连这段外观带参考图整个不拼，画面描述里点名是谁的手、谁的眼就够了。"
              >
