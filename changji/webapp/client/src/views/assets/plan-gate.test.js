...
describe('设定页没有「章节」那一格了', () => {
  // 章节的人和地方在故事页和角色格里都看得到，那一格（AssetEpisodes.vue）
  // 连同预告片那截一起下了。2026-09-18 起连"切成几段"这件事本身都没有了：
  // 一章就是一部分，整章都在。
  it('文件没了', () => {
    expect(
...
describe('补全项目分镜这件事在「这一章」那一页', () => {
  ...
  it('四步一次点完，而且中途按停下就不再往下发', () => {
    ...
    // 景图，再回来跑」——一颗「跑完整部电影」把人支去另一页按两个按钮再走
  ...
  it('补参考图必须排在补分镜之后', () => {
    // 角色和场景是补分镜那一步顺带定下来的（batch.cpp 里那句
