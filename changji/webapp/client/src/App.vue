 ...
 * 外壳。**只有一条。**
 *
 * 原来是六层叠在一起：顶栏、左侧 StepRail、ContextBar（项目名 + 章号）、
 ...
 *
 * 现在一条顶栏装下全部：品牌、项目、五步导航（带对勾）、章号、引擎灯、
 * 主题、设置。**导航只画一遍**。
 *
 * 章号只在「这一章」那一步出现。全片那几步摆一个「当前章」，会让人以为
 * 角色和场景也要每章重做一遍——这条是从原来的 ContextBar 继承下来的判断。
 */
...
/**
 * 这一条和别的章节记录挂在同一章上时，把章号显示出来。
 *
 * 「章节表里一章只该有一条」立起来之前留下的重复：两条挂同一章、标题和
 * 镜头数都一样，下拉里两行一模一样。引擎那头 sync_episodes_to_chapters 早就
 * 把这种算进 `orphans` 回给调用方了，只是**前端一个字都没读**——所以人只
 * 看到两条重复。
 *
 * 不撞车就返回空串：平时不给每一行加长。
 */
 ...
 * 于是在手机上打开「这一章」——四步里的最后一步、也是逐章干活的入口
 ...
 * 挂载钩子；而这一份（项目、章号、侧边那几个对勾、每一章多少镜）是**每一页
 * 都在读**的，砸了之后停在上一次那一章的样子上，要等下一次换项目 / 换章 /
 ...
 * 为空有两种情形，没有一种是"这部电影没起名字"：
 *
 *   · `/bff/flow` 还没回来——每次进页面都有那么一小会儿，顶栏先闪一下
 *     「未命名项目」再变成真名；
 *   · flow 回了 400/404（项目被删了、挪了位置）——那时候它是**长期**空的，
 *     顶栏就一直挂着「未命名项目」，而那部电影其实好好地叫着别的名字。
 *
 * 而「未命名项目」说的偏偏就是"这部电影没起名字"。同一件事 JobBadge 的
 ...
 *
 * 而理解故事那一件活干的是「提结构 → 定长相 → 落成章节 → **逐章写剧本**」
 ...
 *
 * 换成 `counters.understood`（引擎那头 = 故事里有人 + 库里有人 + 至少一章
 * 对上了 + 每一章都写出了剧本）。它正好是「理解故事跑完了」。
 */
...
/**
 * 顶栏下拉里一章的名字：按 `chapter_refs` 里的章号说，不按 `episode_id` 说。
 * `ep01` 是历史留下的名字，老项目里两个号还对不上（见上面 dupChapterNote），
 * 而人认的是「第几章」。
 */
...
// 换项目、换章都要重新算一遍进度，否则导航上的对勾会停在上一个项目上
watch(
...
<template>
  <div class="shell">
    <header v-if="!chromeOff" class="topbar">
      ...
      <!-- **写「这一部电影」不写「换个项目」。** 它去的是 /project，而那一页
           是这一部电影的进度和设置（画幅、模型、删除），根本没有项目列表——
      ...
      <label v-if="perEpisode && session.hasProject" class="ep">
        ...
        >
          ...
          <!-- **两条章节记录挂同一章时要能分辨。** 章节表里一章只该有一条，
               是现在的规矩；而规矩立起来之前留下的重复在这儿长得一模一样：
               2026-09-16 实见 ep08 和 ep09 都写着「了结（17 镜）」，选哪个全靠猜。
               只在真撞车时才多显示那一句，平时不加长。 -->
...
@media (max-width: 860px) {
  /* 项目名和章号让位给导航——导航是一直要用的，那两个是偶尔换一次。
     它们仍然点得到：项目在第一步那一页，章号在「这一章」页里也有。 */
  .proj {
    display: none;
  }
  /* **章号让位给导航，而不是反过来。** 上面那句话原来只兑现了一半：
     `.ep` 是 `flex: none`，一步都不让；真正被挤掉的是 `.nav`（它
     `min-width: 0` 又能横滚，缩起来没有下限）。375px 上进「这一章」那一页
