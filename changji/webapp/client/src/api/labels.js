 ...
 * 这个成片文件属不属于这一章。
 *
 * ⚠️ **不能用 `name.includes(episodeId)`。** 章号到 99 以内是 `ep%02d`，
 * **第 100 章起变成 `ep100`、`ep101`……**（story_plan.cpp 的 `ep_id` 和
 * http/episodes.cpp 的 `ep_fmt` 都是这么写的，那是刻意留的）。于是站在
 * `ep10` 上时，`"ep107.mp4".includes("ep10")` 是真——`ep100` 到 `ep109`
 * 十个成片文件全算成 ep10 的。一部长篇改编下来七八十上百章是常事，这不是
 * 假想的数。
 *
 * 后果不是少一个数，是**审片页会播错片而且不吭声**：`load()` 默认选
 * `forThisEpisode[0]`（片单按 mtime 倒序），完全可能选中 ep107 那条；
 * 而 `isMine` 用的是同一个判据，于是它也说"这是你这一章的"——那句
 * 「播着别的章的片，一个字都不说」的守卫正好被同一个 bug 绕过去。
 * 底下那条跳转条画的永远是 ep10 的镜头，点一下按 ep10 的时长跳。
 *
 * 判据改成**两边都要挨着非字母数字**（或者文件名的头尾）：
 *
 *   ep01.mp4      / ep01  → 真（后面是 `.`）
 *   ep01_2k.mp4   / ep01  → 真（后面是 `_`，手动超分出来的那份还算这一章）
 ...
 * 同一屏上**：这一章那排 tab 写「剧本 157 字」，正下方工具条写「129 字 · 目标
 * 60 秒」。同一份稿子，差 28。
 *
 * 两边各对了一半：
 *
