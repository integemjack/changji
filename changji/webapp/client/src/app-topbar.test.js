 ...
 * "手机上看不出自己在第几步"，而且不报错。
 *
 *   一、章号让位给导航，不是反过来。窄屏那段 media query 的注释一直这么
 *       写着，可 `.ep` 是 `flex: none`——一步都不让；真正被挤掉的是 `.nav`
 *       （`min-width: 0` 又能横滚，缩起来没有下限）。375px 上进「这一章」
 *       那一页时导航只剩 44px，四步里一格都摆不下。
 *   二、打开一页要停在当前那一步上。滚动位置一直是 0 的话，手机上打开
 *       「这一章」——四步里的最后一步——那一格在可视区外七十多像素。
 *
...
describe('流程读不出来的时候，导航别装作"一步都没做"', () => {
   ...
   * 的点落回第一步。一部写完了故事、出完了设定的电影，在引擎抽一下的时候
   * 看着像刚建。
   *
