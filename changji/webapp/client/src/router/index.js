 ...
 *   逐章（每章重复）  这一章
 *
 * 前三步的产物是整部电影共用的：故事是全片讲什么、分几章，设定是全片同一批
 * 人和地方。最后一步是对着某一章干活，顶栏的章号决定在哪一章上操作——
 * `phase` 这个字段就为这个留着（App.vue 的 perEpisode）：全片那几步摆一个
 * 「当前章」，会让人以为角色和场景也要每章重做一遍。
 *
 ...
 *
 * 角色和场景为什么都放在同一个全片资产库里、而不是每章一份：
 * 分镜表里只存 id，外观描述由程序从库里拼接，逐字节相同——这是角色和
 * 场景跨镜头一致的唯一手段。每章一份拷贝的话，同一个安保室在第一章和
 * 第五章会长得不一样，而这正是这套系统要防的事。所以库是全片的，
 * 场景这一步只是把镜头对准「这一章用到的那几个」。
 */
...
export const STEP_ROUTES = [
  ...
  // 梗概逐章续写：章数人填、上下文只带前三章，于是没有全局结构、写到
  // 第五章开始失忆、故事也没有终点。故事这一步先把完整故事和章节定下来。
  {
    key: 'story',
    path: '/story',
    name: 'story',
  ...
  //
  // 顺带修掉场景那一页的错位：它原来挂在「逐章」阶段、标题写着「这一章
  // 在哪儿拍」，而**场景库是全片共用的**——分镜表里只存 id，外观从库里
  // 拼接，这是跨镜头一致的唯一手段。
  {
    key: 'assets',
    path: '/assets',
  ...
  // **镜头、成片、上传合成一步「这一章」**（2026-09-11）。
  //
  // 2026-09-10 已经把分镜和制作合过一次，理由是「人的动作是看片子→改台词→
  // 重出，在同一镜上来回，换页就断了」。这一次是同一条理由再往外一层：
  // 人做的事是**对照着看**——对着这句台词看这一镜对不对，看完整章顺手发
  // 出去。分成四页，来回换页才知道这一镜出自哪句话。
  //
  // 剧本也搬了进来：全片那半（梗概、章节计划、章节正文）在故事页，这里只剩
  // 「这一章的剧本」，和它的镜头摆在同一页上。
  //
  // 侧边栏因此从八格变六格。`/bff/flow` 那边同步合成一个 `episode`，
  // 判据取原来「成片」那条——装配出这一章的成片才算这一章做完了，发布是
  // 可选的收尾动作，没发也不该让这一格一直不打勾。
  {
    key: 'episode',
    path: '/episode',
    name: 'episode',
  ...
  // 成片：整部电影的最后一步。出了片的章按顺序接成一条，一部电影一个文件。
  // **有一章出了片这一步就出现**（App.vue 的 filmedChapters > 0），不等齐——
  // 接一部只有前三章的电影出来看看，本来就是想做的事。
  {
    key: 'film',
    path: '/film',
    name: 'film',
...
const routes = [
  ...
  // 任务页面。**不在 STEP_ROUTES 里**：它不是"做电影的第几步"，是随时
  ...
  // 「剧本大纲」那一页 2026-09-11 删了：全片那半（梗概、章节计划、章节正文）
  // 在故事页，单章那半在「这一章」的剧本视图。预告片和手动加一章当时收进了
  // 设定页「分集」那一格的折叠区，而那一格 2026-09-18 整个下了——改名、复制、
  // 删除搬到了「这一章」页上，预告片今天界面上没有入口（见 JobBadge 的
  // `trailer`）。一页同时干六件事，那一页就不会有重点。
