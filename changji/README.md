...
后两步各自还分几格：**设定**是角色 / 场景 / 分集，**这一集**是剧本 /
镜头 / 成片。原来是八步，2026-09-11 到 09-14 一路合下来——「剧本大纲」
那一步删了（它逐集续写、写到第五集开始失忆，换成先定完整故事的「故事」），
角色和场景本来就在同一个资产库里所以并成「设定」，而分镜到上传那几步
人是对照着看的，并成「这一集」。合并的理由逐条写在
`webapp/client/src/router/index.js` 里。

每一步都能让大模型代劳，每一步的结果都看得见。电脑和手机都能开。

**配置分两处**，按"属于这台机器"还是"属于这部剧"分：设置页管体检、
引擎地址、配音后端、模型目录和下载源、装配与闸门；而挑哪几个模型跑、
大模型的接口地址和密钥、画幅画风，都在项目页——一部剧一份，一台机器上
可以同时有竖屏短剧和横屏片子。
